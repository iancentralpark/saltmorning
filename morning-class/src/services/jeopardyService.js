...
async function generateJeopardyBoard(opts) {
  ...
  const options = {
    model: process.env.TEACHER_GEMINI_MODEL || process.env.GEMINI_MODEL || 'gemini-2.5-flash-lite',
  ...
  let result;
  try {
    result = await askGemini(buildPrompt(opts), [], options);
  } catch (e) {
    const err = new Error(e.message || 'AI generation failed.');
    err.statusCode = 502;
    throw err;
  }
  if (!result || !result.ok) {
