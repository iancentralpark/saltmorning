...
export function DemoAuth() {
  ...
  if (session) {
    return (
      <div className="hidden items-center gap-1.5 sm:flex">
        <span className="max-w-[9rem] truncate text-[11px] font-semibold text-moss-700">
          {session.orgName || session.orgCode}
        </span>
        <button
          type="button"
          disabled={busy}
  ...
  return (
    <div className="relative hidden sm:block">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-1 min-w-[12rem] border border-ink-900/10 bg-sand-50 p-2 shadow-sm">
